...
# Keep only stations in QC with hourly data and long/lat.
stns <- stns[
    prov     == "QC"   &
    interval == "hour" &
    !is.na(lat)        &
...
stns_sf_mtl <- stns_sf[stns_qc_sf$RSS_CODE %in% c("06", "13"), ]
...
qs::qsave(
    x    = stns_mtl,
    file = "data/eccc/stns_mtl_list.qs"
)
